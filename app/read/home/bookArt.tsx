...
// Shared book art for the v4 clean UI — a stable spine color + a simple white
// motif, derived from the book id. Used by the home shelf covers (HomeBookCover)
// and the reader scene panel (ReaderScene) so a book looks like itself
// everywhere. Colors are returned as hex so they work in any CSS scope.
...
export const SPINE_HEX: Record<Spine, string> = {
  forest: '#4E7A54',
  denim: '#4A668A',
  plum: '#7C4B6C',
  teal: '#3B7A80',
...
const ORDER: Spine[] = ['mustard', 'denim', 'forest', 'brick', 'teal', 'plum', 'clay', 'slate']

/** Stable, varied spine color from the id so a shelf reads as a colorful
 *  bookcase (semantic meaning is carried by the motif, not the hue). */
export function spineHexFor(book: { id: string }): string {
  let h = 0
  for (let i = 0; i < book.id.length; i++) h = (h * 31 + book.id.charCodeAt(i)) | 0
  return SPINE_HEX[ORDER[Math.abs(h) % ORDER.length]]
}
...
/** Motif paths, drawn in `currentColor` (set white by the caller). */
export function MotifSvg({ kind }: { kind: Motif }) {
