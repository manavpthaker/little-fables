// Grown-ups — math gate + Parent Corner (landscape). shadcn neutrals, Inter,
// quiet and plain: no emoji, no exclamation points, separate from the kid world.
...
// Lucide glyphs (parent surfaces use the Lucide set — exact paths, stroke style)
...
const IconPencil = (p) => <Lucide {...p} d={<React.Fragment><path d="M21.174 6.812a1 1 0 0 0-3.986-3.987L3.842 16.174a2 2 0 0 0-.5.83l-1.321 4.352a.5.5 0 0 0 .623.622l4.353-1.32a2 2 0 0 0 .83-.497z" /><path d="m15 5 4 4" /></React.Fragment>} />;
...
function GateScreen({ onBack, onPass }) {
  ...
  return (
    <div data-screen-label="Grown-ups — math gate" style={{ height: '100%', background: 'var(--lf-p-muted)', color: 'var(--lf-p-foreground)', fontFamily: 'var(--font-ui)', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0 40px 90px' }}>
...
// ---- Parent corner ----
const REC_INIT = [
  { id: 'r1', story: 'Miko and the Wobbly Bridge', date: 'Jul 8', dur: 42 },
  { id: 'r2', story: 'Azi’s Little Bhen', date: 'Jul 5', dur: 63 },
  { id: 'r3', story: 'Jujy’s Christmas Adventure', date: 'Jul 2', dur: 51 },
];
...
function StoryRow({ s }) {
  const cover = s.id ? window.SW_STORIES[s.id].coverImage : null;
  const draft = s.status === 'Draft';
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '11px 0' }} data-comment-anchor={draft ? 'sw-writing-slot' : undefined}>
      {cover ? (
        <img src={cover} alt="" style={{ width: 44, height: 44, borderRadius: 8, objectFit: 'cover', border: '1px solid var(--lf-p-border)', flexShrink: 0 }} />
      ) : (
        <span style={{ width: 44, height: 44, borderRadius: 8, border: '1px dashed var(--lf-p-border)', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', color: 'var(--lf-p-muted-foreground)', flexShrink: 0 }}>
          <IconPencil size={15} />
        </span>
      )}
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ font: '500 14px/1.4 var(--font-ui)' }}>{s.title}</div>
        <div style={{ font: '400 12.5px/1.4 var(--font-ui)', color: 'var(--lf-p-muted-foreground)' }}>{s.by} · {s.pages} pages · teaches {s.teaches}{draft ? ' · art still painting' : ''}</div>
      </div>
      <span style={{ font: '500 11.5px var(--font-ui)', color: draft ? 'var(--lf-p-muted-foreground)' : 'var(--lf-p-foreground)', border: '1px solid var(--lf-p-border)', background: draft ? 'var(--lf-p-muted)' : 'var(--lf-p-background)', borderRadius: 999, padding: '3px 10px', flexShrink: 0 }}>{s.status}</span>
    </div>
  );
}
...
function ParentScreen({ onBack }) {
  ...
  return (
    <div className="sw-screen" data-screen-label="Parent corner" style={{ background: 'var(--lf-p-muted)', color: 'var(--lf-p-foreground)', fontFamily: 'var(--font-ui)' }}>
      <div style={{ maxWidth: 1020, margin: '0 auto', padding: '22px 32px 48px' }}>
        <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between' }}>
          <div>
            <PButton variant="ghost" onClick={onBack} style={{ color: 'var(--lf-p-muted-foreground)', marginLeft: -12 }}>
              <IconChevronLeft /> Back to Story World
            </PButton>
            <h1 style={{ margin: '14px 0 4px', font: '700 26px/1.3 var(--font-ui)', letterSpacing: '-0.02em' }}>Parent corner</h1>
            <p style={{ margin: 0, font: '400 14px/1.5 var(--font-ui)', color: 'var(--lf-p-muted-foreground)' }}>Azad · 4 years old · 12 stories read this month</p>
          </div>
          <PButton><IconPlus /> New story</PButton>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginTop: 22, alignItems: 'start' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            <PCard title="Stories" description="The library your family makes for Azad.">
              <div style={{ display: 'flex', flexDirection: 'column' }}>
                {window.SW_PARENT_STORIES.map((s, i) => (
                  <div key={s.title} style={{ borderTop: i === 0 ? 'none' : '1px solid var(--lf-p-border)' }}>
                    <StoryRow s={s} />
                  </div>
                ))}
              </div>
          ...
          {/* Right column: universe editor */}
          <PCard title="Azad’s universe" description="What new stories draw from. Changes apply to the next story.">
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, font: '400 12px var(--font-ui)', color: 'var(--lf-p-muted-foreground)', justifyContent: 'center', marginTop: 18 }}>
