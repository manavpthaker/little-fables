// Story World — router, landscape device frame, tweaks wiring.
const { HomeScreen, ReaderScreen, EndScreen, GateScreen, ParentScreen } = window;
...
const SW_STATE_KEY = 'sw-state-v2';
const ROUTES = ['home', 'reader', 'end', 'gate', 'parent'];

function loadState() {
  try {
    const s = JSON.parse(localStorage.getItem(SW_STATE_KEY) || '{}');
    if (s && typeof s === 'object' && ROUTES.includes(s.route || 'home')) return s;
  } catch (e) { /* fresh start */ }
  return {};
}

// ---- iPad-landscape device frame, scaled to fit the viewport ----
const DEV_W = 1180, DEV_H = 820, BEZEL = 21;
...
function StoryWorldApp() {
  ...
  React.useEffect(() => {
    localStorage.setItem(SW_STATE_KEY, JSON.stringify({ route, storyId, bedtime }));
  }, [route, storyId, bedtime]);
  ...
  if (route === 'reader') {
    ...
    screen = <HomeScreen bedtime={bedtime} offline={t.offline}
