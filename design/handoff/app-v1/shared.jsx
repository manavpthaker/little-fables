...
function Doodles() {
  const marks = [
    { ch: '✦', top: 170, left: 26, size: 30 },
    { ch: '✦', top: 560, right: 22, size: 24 },
    { ch: '〰', bottom: 300, left: 20, size: 28 },
  ];
  return (
    <React.Fragment>
      {marks.map((m, i) => (
...
function PillNav({ active, onNav }) {
  ...
  return (
    <nav style={{ position: 'absolute', left: 90, right: 90, bottom: 22, background: 'var(--lf-cream-card)', border: '1.5px solid var(--lf-cream-line)', borderRadius: 'var(--radius-pill)', boxShadow: 'var(--shadow-warm)', display: 'flex', alignItems: 'center', justifyContent: 'space-around', padding: '10px 12px', zIndex: 10 }}>
      {items.map((it) => {
        ...
        return (
          <button key={it.id} type="button" className="lf-press" onClick={() => onNav(it.id)}
            style={{
              ...
              borderRadius: 'var(--radius-pill)', padding: isActive ? '13px 24px' : '13px 16px',
